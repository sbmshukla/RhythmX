...
export const api = {
  ...
  async convertVideo(url, playlistId) {
    ...
    try {
      ...
      // Track data usage immediately by doing a zero-byte HEAD request to get file size
      try {
        if (data && data.download_url) {
          const headRes = await fetch(`${API_BASE}${data.download_url}`, { method: 'HEAD' });
          const size = headRes.headers.get('Content-Length');
          if (size) {
            dataTracker.addUsage(url, parseInt(size, 10));
          }
        }
      } catch (err) {
